...
function main() {
  ...
  try {
    ...
    console.error(
      'Не удалось восстановить публичный ключ из приватного:',
      e.message,
    );
    process.exit(1);
  }
  ...
  if (derived !== publicKey) {
    console.error(
      'ОШИБКА: WEB_PUSH_PUBLIC_KEY не соответствует WEB_PUSH_PRIVATE_KEY.',
    );
    console.error(
      '  из .env public:',
    ...
    console.error(
      '  из private ECDH:',
      `${derived.slice(0, 16)}…${derived.slice(-12)}`,
    );
    console.error(
      'Сгенерируй пару заново: npm run push:keys и обнови оба ключа в .env.',
    );
    process.exit(1);
  }
  ...
  console.log('');
  console.log(
    'Сверь с живым API (поле publicKey должно совпадать с строкой выше):',
  );
  console.log(
    '  curl -sS -H "Authorization: Bearer <JWT>" "https://<API>/push/public-key"',
  );
  console.log('');
  console.log(
    'iPhone: если ключи меняли после подписок — в профиле снова «Подключить push»',
  );
