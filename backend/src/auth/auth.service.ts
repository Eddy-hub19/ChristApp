...
export class AuthService {
  ...
  private requestMeta(req: Request): string {
    ...
    const forwardedHost =
      req.headers['x-forwarded-host'] || 'no-x-forwarded-host';
  ...
  async refresh(req: Request, res: Response) {
    ...
    if (!raw || typeof raw !== 'string') {
      this.logger.warn(
        `refresh denied: no refresh cookie; ${cookieInfo}; ${meta}`,
      );
      throw new UnauthorizedException();
    }
  ...
  private async buildAccessResponse(userId: string) {
    ...
    const signedToken = this.jwt.sign(payload, {
      expiresIn: this
        .accessExpiresIn as import('jsonwebtoken').SignOptions['expiresIn'],
    });
