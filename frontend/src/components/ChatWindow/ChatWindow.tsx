...
function ChatWindow({
  ...
  const formatTypingLine = (
    statuses: Array<{ username: string; activity: "text" | "voice" }>,
  ) => {
  ...
  const estimateScrollDownThresholdPx = useCallback(() => {
    ...
    const gapHeight =
      Math.max(0, lastMessages.length - 1) * CHAT_VERTICAL_GAP_PX;
  ...
  const updateScrollDownVisibility = useCallback(() => {
    ...
    shouldFollowBottomRef.current =
      distanceFromBottom <= AUTO_SCROLL_FOLLOW_DISTANCE_PX;
        ...
        return (
          <MessageBubble
            ...
            showReadReceipt={Boolean(
              readReceiptUsers.length ||
              (readReceiptMessageId && readReceiptMessageId === message.id),
            )}
