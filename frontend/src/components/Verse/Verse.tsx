...
function parseVerseKeyForSave(
  verseKey: string,
): { bookName: string; chapter: number; verse: number; text: string } | null {
...
function Verse({
  ...
  const handleHighlightColorSelect = (
  ...
  const selectedFromPreset = Boolean(
    ...
    <div className={styles.verseContainer} id={id}>
      <div className={styles.verseRow}>
        <p className={styles.verse} onClick={handleClick}>
          ...
          >
            <sup
              className={verse === 1 ? styles.verseSupLarge : styles.verseSup}
            >
              {verse}
            </sup>
