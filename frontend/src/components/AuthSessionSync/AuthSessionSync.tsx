...
export default function AuthSessionSync() {
  ...
  useEffect(() => {
    ...
    const runInitialSync = async () => {
      if (
        process.env.NEXT_PUBLIC_DEBUG_PWA_AUTH === "1" &&
        isPwaStandaloneClient()
      ) {
        try {
          window.alert(
            `Debug LS: ${Boolean(window.localStorage.getItem("christ_access_token"))}`,
          );
        } catch {
          // ignore
        }
  ...
  return (
    <div
      ...
      style={{
        ...
        placeItems: "center",
        background:
          "radial-gradient(circle at 30% 20%, #31251d 0%, #1a1715 55%, #0f0e0d 100%)",
        color: "#f5ecde",
      }}
    >
