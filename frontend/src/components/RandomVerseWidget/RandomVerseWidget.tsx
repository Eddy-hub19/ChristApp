...
export default function RandomVerseWidget({
  ...
  const bibleTranslation = useMemo(() => {
    ...
    return pickTranslationShortName(
      translations as BibleTranslationItem[],
      lang,
    );
  }, [lang, translations]);
  ...
  const rootClass = embedInGreeting
    ? `${styles.container} ${styles.embedInGreeting}`
    : styles.container;
