...
export default function CallScreen({
  ...
  const remoteTracksRef = useRef<
    Map<string, IAgoraRTCRemoteUser["audioTrack"]>
  >(new Map());
    ...
    const startCall = async () => {
      ...
      try {
        ...
        const response = await apiFetch(
          `${HTTP_API}/calls/token?channelName=${encodeURIComponent(channelName)}`,
          {
            headers: {
        ...
        client.on("volume-indicator", (volumes) => {
          const isRemoteSpeaking = volumes.some(
            (item) => Number(item.uid) !== joinUid && item.level > 6,
          );
