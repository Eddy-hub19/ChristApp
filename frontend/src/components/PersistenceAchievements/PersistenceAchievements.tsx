...
export function countUnlockedPersistenceAchievements(
  streak: number,
  savedVersesCount = 0,
): number {
  return PERSISTENCE_ACHIEVEMENTS.filter((a) =>
    isAchievementUnlocked(a, streak, savedVersesCount),
  ).length;
}
    ...
    <section className={styles.section} aria-labelledby="persistence-heading">
      ...
      <div className={styles.grid}>
        {PERSISTENCE_ACHIEVEMENTS.map((a) => {
          const isActive = isAchievementUnlocked(
            a,
            dayStreak,
            savedVersesCount,
          );
          ...
          const progressLeft =
            "savedVerses" in a
              ? Math.max(0, a.savedVerses - savedVersesCount)
              : Math.max(0, a.days - dayStreak);
          const lockLabel =
            "savedVerses" in a
              ? `Ещё ${progressLeft} стих.`
              : `Ещё ${progressLeft} дн.`;
          ...
          return (
            ...
            >
              ...
              >
                {isActive ? (
                  <div className={styles.ringSpin} aria-hidden />
                ) : null}
                ...
                <div className={styles.cardInner}>
                  ...
                  <motion.span
                    ...
                    animate={
                      justUnlocked ? { scale: [1, 1.1, 1] } : { scale: 1 }
                    }
                    transition={{
                      duration: 0.65,
                  ...
                  {!isActive ? (
                    <p className={styles.lockHint}>{lockLabel}</p>
                  ) : null}
                </div>
              </div>
            </motion.article>
