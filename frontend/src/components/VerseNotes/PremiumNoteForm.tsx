...
function CornerPenIcon({ animating }: { animating: boolean }) {
  return (
    <div
      className={`${styles.cornerPen} ${animating ? styles.cornerPenWriting : ""}`}
      aria-hidden
    >
      <svg
        className={styles.cornerPenSvg}
        viewBox="0 0 24 24"
        fill="none"
...
export default function PremiumNoteForm({
  ...
  const [focusedSlot, setFocusedSlot] = useState<null | "source" | "response">(
    null,
  );
  ...
  const handleFieldBlur = useCallback(() => {
    requestAnimationFrame(() => {
      ...
      if (
        a !== sourceAutosize.ref.current &&
        a !== responseAutosize.ref.current
      ) {
        setFocusedSlot(null);
      }
    });
