...
const VARIANT_ICON: Record<
  VerseNotesCollectionMeta["coverVariant"],
  React.ReactNode
> = {
...
export default function CollectionCoverCard({
  collection,
}: {
  collection: VerseNotesCollectionMeta;
}) {
  const blurClass = VARIANT_CLASS[collection.coverVariant];
  const icon = VARIANT_ICON[collection.coverVariant];

  return (
    <Link href={`/verse-notes/${collection.id}`} className={styles.link}>
