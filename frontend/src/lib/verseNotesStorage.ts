import {
  VERSE_NOTES_COLLECTIONS,
  type VerseNotesCollectionId,
} from "./verseNotesCollections";
...
export function loadVerseNotes(
  userId: string,
  collectionId: VerseNotesCollectionId,
): VerseNoteRecord[] {
...
export function saveVerseNotes(
  ...
  try {
    localStorage.setItem(
      storageKey(userId, collectionId),
...
export function loadRecentVerseNotesAcrossCollections(
  userId: string,
  limit = 6,
): VerseNoteRecord[] {
