...
export async function fetchChapters(bookId: string, translation: string) {
  ...
  if (
    !currentBook ||
    typeof (currentBook as { chapters?: unknown }).chapters !== "number"
  ) {
    return [];
  }
...
export async function fetchTranslations() {
  ...
  return data.flatMap((lang: { language?: string; translations?: unknown[] }) =>
    (Array.isArray(lang?.translations) ? lang.translations : []).map(
      (t: unknown) => {
        const tr = t as {
          short_name?: string;
          full_name?: string;
...
function isAllowedRandomBook(book: { id?: string; name?: string }) {
  ...
  if (
    name.includes("псалтир") ||
    name.includes("псалом") ||
    name.includes("psalm")
  ) {
    return true;
  }
...
export async function fetchRandomVerse(translation: string) {
  ...
  const randomBook = allowedBooks[
    Math.floor(Math.random() * allowedBooks.length)
  ] as {
    id?: string;
    name?: string;
    chapters?: number;
  ...
  const verses = await fetchFullChapter(
    randomBook.id,
    randomChapter,
    translation,
  );
