...
export function serializeVerseSharePayload(payload: VerseSharePayload) {
  const safePayload: VerseSharePayload = {
    ...
    verses: Array.from(
      new Set(
        payload.verses.filter((v) => Number.isFinite(v)).map((v) => Number(v)),
      ),
    ).sort((a, b) => a - b),
...
export function parseVerseSharePayload(rawContent: string) {
  ...
  const encodedMeta = rawContent.slice(
    VERSE_SHARE_META_PREFIX.length,
    suffixIndex,
  );
  const plainContent = rawContent.slice(
    suffixIndex + VERSE_SHARE_META_SUFFIX.length,
  );

  try {
    const parsed = JSON.parse(
      decodeURIComponent(encodedMeta),
    ) as Partial<VerseSharePayload>;
    ...
    if (!parsed.bookName || !parsed.chapter || verses.length === 0) {
      return {
        payload: null as VerseSharePayload | null,
        content: plainContent || rawContent,
      };
    }
