...
export const BIBLE_READING_PROGRESS_CHANGED_EVENT =
  "christapp:bible-reading-progress";
...
export function readBibleLastReadFromStorage(): BibleLastRead | null {
  ...
  return parseLastRead(
    window.localStorage.getItem(BIBLE_LAST_READ_STORAGE_KEY),
  );
}
...
export function markBibleChapterCompleted(
  bookId: string,
  chapter: number,
): boolean {
