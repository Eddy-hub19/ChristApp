...
function createShareWithJesusChatItem(
  roomId: string,
  shareTitle: string,
...
function orderChatListRows(
  rooms: ChatListItem[],
  sortLang: string,
): ChatListItem[] {
  const shareWithJesusRoom = rooms.find(
    (room) => room.id === SHARE_WITH_JESUS_CHAT_ID,
  );
  const globalRoom = rooms.find((room) => room.id === GLOBAL_ROOM_ID);
  const directRooms = rooms.filter(
    (room) =>
      room.id !== GLOBAL_ROOM_ID && room.id !== SHARE_WITH_JESUS_CHAT_ID,
  );

  const sortedDirectRooms = [...directRooms].sort((leftRoom, rightRoom) => {
    const leftTs = leftRoom.lastActivityAt
      ? new Date(leftRoom.lastActivityAt).getTime()
      : 0;
    const rightTs = rightRoom.lastActivityAt
      ? new Date(rightRoom.lastActivityAt).getTime()
      : 0;
    if (leftTs !== rightTs) {
      return rightTs - leftTs;
    }
    const byTitle = leftRoom.title.localeCompare(rightRoom.title, sortLang, {
      sensitivity: "base",
    });
    if (byTitle !== 0) {
      return byTitle;
...
function prependDirectRoomIfMissing(
  ...
  return shareWithJesusRoom
    ? orderChatListRows(
        [shareWithJesusRoom, globalRoom, newDirect, ...directRooms],
        sortLang,
      )
...
function prependShareWithJesusRoomIfMissing(
  ...
  return orderChatListRows(
    [
      createShareWithJesusChatItem(roomId, shareTitle),
      globalRoom,
      ...directRooms,
    ],
    sortLang,
...
function lastActivityInstantMs(
...
function shouldShowListNotification(
  isActiveRoom: boolean,
  currentPath: string,
) {
  return (
    !isActiveRoom &&
    (document.visibilityState !== "visible" || !isChatPath(currentPath))
  );
}
...
function getDirectServerRoomIdForListRow(
  ...
  return (
    peerToRoom.get(listRowId) ?? peerToRoom.get(canonicalChatUuidKey(listRowId))
  );
}

function readChatListText(
  value: unknown,
  preferredKey: "title" | "preview",
): string {
...
export default function ChatPage() {
  ...
  const [rooms, setRooms] = useState<ChatListItem[]>(() => [
    createGlobalChatItem(GLOBAL_CHAT_TITLE),
  ]);
  ...
  const roomsRef = useRef<ChatListItem[]>([
    createGlobalChatItem(GLOBAL_CHAT_TITLE),
  ]);
  ...
  const activeRoomIdPath = pathname?.startsWith("/chat/")
  ...
  const applyOnlinePresenceToRooms = useCallback(
    (nextOnlineIds: Set<string>) => {
      setRooms((prev) => {
        ...
        const nextRooms = prev.map((room) => {
          ...
          const isOnline =
            nextOnlineIds.has(room.id) ||
            nextOnlineIds.has(canonicalChatUuidKey(room.id));
  ...
  const syncOnlinePresence = useCallback(
    (nextOnlineIds: Set<string>) => {
      ...
      setOnlineUserIds((prev) =>
        areSetsEqual(prev, nextOnlineIds) ? prev : nextOnlineIds,
      );
  ...
  const applyUnreadSummary = useCallback(
    (summary: UnreadSummaryResponse) => {
      const summaryByRoomId = new Map(
        summary.rooms.map((roomSummary) => [
          summaryRoomKey(roomSummary.roomId),
          roomSummary,
        ]),
      );
      ...
      setRooms((prev) => {
        ...
        const nextRooms = prev.map((room) => {
          const rawSourceId =
            room.id === GLOBAL_ROOM_ID
              ? GLOBAL_ROOM_ID
              : getDirectServerRoomIdForListRow(
                  room.id,
                  directUserIdToRoomIdRef.current,
                );
          ...
          const nextUnread = summaryRoom
            ? Math.max(0, Number(summaryRoom.unread) || 0)
            : 0;
          const formattedPreview = summaryRoom?.lastMessage
            ? formatRoomPreviewFromLastMessage(
                rawSourceId,
                summaryRoom.lastMessage,
                user?.username,
              )
            : "";
          const nextPreview =
            formattedPreview.trim() !== ""
              ? formattedPreview
              : (room.preview ?? "");
          ...
          const currentUnread =
            typeof room.unread === "number" ? room.unread : room.unread ? 1 : 0;
        ...
        const hasOrderChanges = orderedRooms.some(
          (room, index) => room.id !== prev[index]?.id,
        );
  ...
  const usersById = useMemo(() => {
    return new Map(
      users.map((existingUser) => [existingUser.id, existingUser]),
    );
  }, [users]);
  ...
  const chatCreateCandidates = useMemo<ChatCreateCandidate[]>(() => {
    ...
    return users
      ...
      .map((existingUser) => ({
        ...
        hasDirectChat: directChatUserIds.has(
      ...
      .sort((leftUser, rightUser) => {
        ...
        return leftUser.handle.localeCompare(rightUser.handle, sortLang, {
          sensitivity: "base",
        });
      });
        ...
        prev.map((room) => {
          if (room.id === GLOBAL_ROOM_ID) {
            return {
              ...
              avatarInitials: getInitials(globalChatTitle),
            };
          }
          if (room.id === SHARE_WITH_JESUS_CHAT_ID) {
            return {
              ...room,
              title: shareWithJesusTitle,
              avatarInitials: getInitials(shareWithJesusTitle),
            };
          }
          return room;
        }),
  ...
  useEffect(() => {
    chatListRuntimeCache = {
      ...
      roomIdToDirectUserId: Array.from(
        roomIdToDirectUserIdRef.current.entries(),
      ),
      directUserIdToRoomId: Array.from(
        directUserIdToRoomIdRef.current.entries(),
      ),
    };
  }, [onlineUserIds, rooms]);
    ...
    queueMicrotask(() => {
      ...
      setRooms((prev) => {
        ...
        const nextRooms = prev.map((room) => {
          ...
          const matchedUser =
            usersMap.get(room.id) ??
            usersMap.get(canonicalChatUuidKey(room.id));
          if (!matchedUser) {
            return room;
          }
  ...
  useEffect(() => {
    ...
    const onMyRooms = (data: { rooms: RoomSocketItem[] }) => {
      ...
      const previousById = new Map(
        previousRooms.map((room) => [room.id, room]),
      );
      ...
      const globalRoom = getGlobalRoomFromPrevious(
        previousRooms,
        i18nRooms.globalTitle,
      );
      const previousShareWithJesusRoom =
        getShareWithJesusRoomFromPrevious(previousRooms);
      ...
      socketRooms
        ...
        .forEach((room) => {
          if (isShareWithJesusRoomTitle(room.title)) {
            nextRoomToUser.set(
              canonicalChatUuidKey(room.id),
              SHARE_WITH_JESUS_CHAT_ID,
            );
            nextUserToRoom.set(SHARE_WITH_JESUS_CHAT_ID, room.id);
            shareWithJesusRoom = createShareWithJesusChatItem(
              room.id,
              i18nRooms.shareTitle,
              previousShareWithJesusRoom,
            );
            return;
          }

          const targetUserId = getDirectTargetUserId(
            room.title,
          ...
          const targetUser =
            usersById.get(targetUserId) ?? usersById.get(peerListId);
          const directPeer = room.directPeer;
          const previous =
            previousById.get(targetUserId) ?? previousById.get(peerListId);
          const directTitle =
            targetUser?.nickname ??
            targetUser?.username ??
            directPeer?.nickname ??
          ...
          const resolvedAvatarUrl =
            targetUser?.avatarUrl ?? directPeer?.avatarUrl;
      ...
      const cachedUnread = summaryUserId
        ? queryClient.getQueryData<UnreadSummaryResponse>(
            pushUnreadSummaryQueryKey(summaryUserId),
          )
        : undefined;
      if (cachedUnread) {
        queueMicrotask(() => {
    ...
    const onUserPresenceChanged = (
      payload: UserPresenceChangedSocketPayload,
    ) => {
      ...
      const onlineUnchanged = areSetsEqual(
      ...
      const directRoomId = directUserId
        ? (directUserIdToRoomIdRef.current.get(directUserId) ??
          directUserIdToRoomIdRef.current.get(
            canonicalChatUuidKey(directUserId),
          ))
        : undefined;
      const active = activeRoomIdRef.current;
      const isActiveRoom =
        (active != null &&
          canonicalChatUuidKey(String(active)) ===
            canonicalChatUuidKey(String(mappedId))) ||
        (Boolean(directRoomId) && active === directRoomId);

      const currentPath = pathnameRef.current ?? "";
      const shouldShowNotification =
        !isOwnMessage && shouldShowListNotification(isActiveRoom, currentPath);
      ...
      if (shouldShowNotification) {
        ...
        const mappedRoomId =
          mappedId === SHARE_WITH_JESUS_CHAT_ID
            ? directRoomId || msg.roomId
            : mappedId;
        const notificationTitle =
          mappedId === GLOBAL_ROOM_ID
            ? i18n.notificationGlobal(msg.username ?? i18n.anonymousUser)
            : i18n.notificationDirect(
                mappedRoom?.title ?? i18n.peerInNotification,
              );
      ...
      setRooms((prev) =>
        (() => {
          ...
          const skipSelfDirect =
            currentUserId != null &&
            mappedId !== GLOBAL_ROOM_ID &&
            canonicalChatUuidKey(currentUserId) ===
              canonicalChatUuidKey(String(mappedId));
          const baseRooms =
    ...
    const onRemoveSelfFromRoomResult = (payload: RemoveSelfFromRoomResult) => {
      if (!payload?.ok) {
        window.alert(
          typeof payload?.error === "string"
            ? payload.error
            : chatI18nRef.current.deleteListItemFailed,
        );
        return;
      }
      ...
      const listIdForRoom =
        ...
        roomIdToDirectUserIdRef.current.get(
          canonicalChatUuidKey(removedRoomId),
        );
      if (
        listIdForRoom &&
        activeRoomIdRef.current &&
        canonicalChatUuidKey(activeRoomIdRef.current) ===
          canonicalChatUuidKey(listIdForRoom)
      ) {
        router.push("/chat");
      }
    ...
    const onDirectRoomOpened = (payload: DirectRoomOpenedSocketEvent) => {
      ...
      const peerListId = targetUserId
        ? canonicalChatUuidKey(targetUserId)
        : undefined;
      if (peerListId && peerListId !== canonicalChatUuidKey(user?.id ?? "")) {
        const targetUser =
          ...
          usersRef.current.find(
            (existingUser) =>
              canonicalChatUuidKey(existingUser.id) === peerListId,
          );
        const directTitle =
          targetUser?.nickname ??
          targetUser?.username ??
      ...
      if (payload?.roomId) {
        const nextRouteRoomId =
          payload.roomId === GLOBAL_ROOM_ID ? GLOBAL_ROOM_SLUG : payload.roomId;
  ...
  const handleCreateChat = useCallback(
    (targetUserId: string) => {
      ...
      const existingDirectRoomId =
        ...
        directUserIdToRoomIdRef.current.get(
          canonicalChatUuidKey(targetUser.id),
        );
  ...
  const chatItems = useMemo(() => {
    const base =
      rooms.length > 0
        ? rooms
        : [
            createGlobalChatItem(globalChatTitle, {
              preview: t("noChatConnection"),
            }),
          ];
  ...
  const adminDashboardVisible = useMemo(
