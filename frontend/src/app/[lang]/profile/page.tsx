...
function effectiveThemeForegroundHex(
  user: { themeForegroundHex?: string | null } | null,
): string {
  const raw = user?.themeForegroundHex?.trim() ?? "";
  return /^#[0-9a-f]{6}$/i.test(raw)
    ? raw.toLowerCase()
    : SUGGESTED_THEME_FOREGROUND.toLowerCase();
}
...
export default function ProfilePage() {
  ...
  const dateLangTag =
    lang === "ua" ? "uk-UA" : lang === "en" ? "en-US" : "ru-RU";
  ...
  const [pushSyncErrorMessage, setPushSyncErrorMessage] = useState<
    string | null
  >(null);
  ...
  const verseKeeperProgressUI = hydrated
    ? verseKeeperProgress
    : t("verseProgressInitial", { total: 5 });
  ...
  const deleteVerseMutation = useMutation({
    ...
    onMutate: async (verseId) => {
      ...
      const previous = queryClient.getQueryData<SavedVerse[]>(
        savedVersesQueryKey(),
      );
  ...
  const resetAppearanceMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        throw new Error(
          getApiErrorMessage(errData, t("resetAppearanceFailed")),
        );
      }
      return (await res.json()) as AuthUser;
    },
    ...
    onError: (err, _vars, ctx) => {
      ...
      window.alert(
        err instanceof Error ? err.message : t("resetAppearanceFailed"),
      );
    },
    onSuccess: (serverUser) => {
      replaceUser(serverUser);
  ...
  const handleSaveProfile = () => {
    ...
    const sameTheme =
      hexBg === effBg &&
      themeFontInput === normalizeThemeFontKey(user.themeFontKey);
    ...
    <section className={styles.profile}>
        ...
        <div className={styles.profileHeaderBar}>
          ...
          <div className={styles.headerActions}>
            ...
            <div className={styles.settingsColumn} ref={settingsRef}>
              ...
              <div
                className={`${styles.settingsMenu} ${isSettingsOpen ? styles.show : ""}`}
                id="settings-menu"
                role="menu"
              >
      ...
      <div className={styles.userInfo}>
        ...
        <div className={styles.info}>
          ...
          {user?.bio?.trim() ? (
            <p className={styles.userBio}>{user.bio.trim()}</p>
          ) : null}
          <span>{t("memberSince", { date: formattedDate })}</span>
          <span className={styles.avatarLikesLine}>
            {t("avatarLikesReceived", {
              count: myAvatarLikes?.receivedCount ?? 0,
            })}
          </span>
        </div>
      </div>
        ...
        <div className={styles.profileEdit}>
          ...
          <label className={styles.profileLabel}>
            ...
            <span className={styles.colorRow}>
              <input
                ...
                type="color"
                value={
                  themeBgInput.startsWith("#") && themeBgInput.length === 7
                    ? themeBgInput
                    : SUGGESTED_THEME_BACKGROUND
                }
          ...
          <label className={styles.profileLabel}>
            ...
            <select
              ...
              onChange={(event) =>
                setThemeFontInput(event.target.value as ThemeFontKey)
              }
            >
      ...
      <ul className={styles.list}>
        ...
        <li
          className={[
            styles.item,
            styles.rewardItem,
            verseKeeperUnlockedUI ? styles.rewardItemUnlocked : "",
          ]
            .filter(Boolean)
            .join(" ")}
        >
          <Image
            className={[
              styles.imgIcon,
              verseKeeperUnlockedUI ? styles.rewardIconUnlocked : "",
            ]
              .filter(Boolean)
              .join(" ")}
            src={"/icon-badge.svg"}
      ...
      <div className={styles.savedVersesSection}>
        <div className={styles.sectionHeader}>
          <span>
            {t("savedVerses", { count: hydrated ? savedVerses.length : 0 })}
          </span>
        </div>
              ...
              <div key={verse.id} className={styles.verseItem}>
                <div className={styles.verseContent}>
                  ...
                  <span className={styles.savedDate}>
                    {hydrated
                      ? new Date(verse.savedAt).toLocaleDateString(dateLangTag)
                      : "\u2014"}
                  </span>
                </div>
                <button
